...
function Card({ movie }: { movie: Movie }) {
    ...
    const [ratings, setRatings] = useState(movie.Ratings ? Math.round(movie.Ratings * 100) / 100 : 0);
    const [userEst, setUserEst] = useState<number>(0);
    useEffect(() => {
        axios.get(`/api/v1/ratings/?movie_id=${movie.id}&user_id= 1`).then(res => {
            if(res.data.Response!="False"){
                setUserEst(res.data.data.rating);
            }
        }).catch(err => {
            console.log(err);
        })
    ...
    function setRatingUser(e:number) {
        ...
        axios.post('/api/v1/ratings', { movie_id: movie.id, rating: e,user_id: 1 }).then(res => {
            if(res.data.Response!="False"){
                setRatings(Math.round(res.data.Ratings * 100) / 100);
            }
        }).catch(err => {
            console.log(err);
        })
...

                                {
                                    norEst.map((est, index) => {
                                        return (
                                            ...
                                            <svg key={index} onClick={()=>setRatingUser(index+1)} className={'w-10 h-10 text-gray-300 hover:text-yellow-300 cursor-pointer me-1 hover:scale-125 transform-gpu '+(index == 4 ? 'dark:text-gray-500 ':' ')+( userEst>index ? 'text-yellow-300 dark:text-yellow-500':' ')} aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 22 20">
